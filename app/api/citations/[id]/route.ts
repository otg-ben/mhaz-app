...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  if (citation.user_id !== user.id) return NextResponse.json({ error: 'Forbidden' }, { status: 403 })
  if (Date.now() - new Date(citation.created_at).getTime() > 24 * 60 * 60 * 1000) {
    return NextResponse.json({ error: 'Edit window expired' }, { status: 403 })
  }

  const body = await req.json()
  const allowed = ['agency', 'description', 'incident_date'] as const
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const { error } = await supabase.from('citations').delete().eq('id', params.id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
